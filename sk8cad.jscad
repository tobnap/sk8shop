...
function getParameterDefinitions() {
	return ([
		...
		{ name: 'display_item', caption: 'Show ', type: 'choice', values: [0, 1, 2, 3, 4, 5], initial: 0, captions: ['Board', 'Male Mold', 'Female Mold', 'Inverted Male Mold', 'Inverted Female Mold', 'Both'] },
		...
		{ name: 'mold_hole_support', type: 'checkbox', checked: true, caption: 'Truck Holes Supports' },
		...
		*/
		...
		{ name: 'slice_equally', type: 'checkbox', checked: false, caption: 'Slice Equally' },
		...
		{ name: 'ziptie_recess_depth', type: 'float', initial: 10/25.4, step: 0.05, min: 0, max: 5, caption: 'Ziptie Recess Depth, in <span id="boreSpacingInches" class="inchDisplay" > </span>' },
		{ name: 'ziptie_recess_width', type: 'float', initial: 9/25.4, step: 0.05, min: 0, max: 5, caption: 'Ziptie Recess Width, in <span id="boreSpacingInches" class="inchDisplay" > </span>' },
...
function main(parameters) {
	...
	var mold_hole_support = parameters.mold_hole_support;
	...
	var ziptie_recess_depth = parameters.ziptie_recess_depth;
	var ziptie_recess_width = parameters.ziptie_recess_width;
	...
	while (a == 1) {
		...
		if (display == '0' || showProfile == true) {
			...
			holes = make_wheelbase(bolt_pattern_length, bolt_pattern_width, wheelbase, hole_depth, truck_hole_diameter).translate([0, 0, -3 * (mold_height + mold_offset + min_cavity_height) / 2]);
			...
			skateboard = skateboard.subtract(holes);
		}

		//skateboard = color([0,0.99,0],skateboard);

		var slice_equally = parameters.slice_equally;
		var printLength;
		var printWidth;

		if (slice_equally) {
			printLength = (skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/5;
			printWidth = (skateboard.getBounds()[1].y - skateboard.getBounds()[0].y)/2;
		} else {
			printLength = parameters.print_length;
			printWidth = parameters.print_width;
		}
		...
		if (make_mold_zipties == true && !boardDisplay) {
			skateboard = doZipties(skateboard, display, router_template, nose_length, tail_length, mold_length, mold_width, mold_height, mold_offset, printLength, printSection, nose_nose_male_ziptie, anose_nose_male_ziptie, center_male_ziptie, atail_tail_male_ziptie, tail_tail_male_ziptie, nose_nose_female_ziptie, anose_nose_female_ziptie, center_female_ziptie, atail_tail_female_ziptie, tail_tail_female_ziptie, ziptie_recess_depth, ziptie_recess_width);
		}

		if (boardDisplay == true && slice_equally) {
			var profile = make_profile(width, wheelbase, bolt_pattern_length, nose_length, tail_length, length, taperN, taperT, nose_shape, tail_shape, mold_height);
			skateboard = doPrintSections(skateboard, display, mold_length, mold_height, mold_offset, min_cavity_height, printSection, (profile.getBounds()[1].x - profile.getBounds()[0].x)/5, sectionWidth, nose_length, tail_length, sectionWidth/2);	
		} else {
			skateboard = doPrintSections(skateboard, display, mold_length, mold_height, mold_offset, min_cavity_height, printSection, printLength, sectionWidth, nose_length, tail_length, sectionWidth/2);
		}
    ...
    //modelArray[0] = doPrintSections(modelArray[0], display, mold_length, mold_height, mold_offset, min_cavity_height, printSection, printLength, -modelArray[0].getBounds()[0].y, nose_length, tail_length, -modelArray[0].getBounds()[0].y/2);

    if (export_in_mm == true) {
        modelArray[0] = modelArray[0].scale(25.4);
    }
	...
	if (display == '2' || display == '4') {
		...
		switch (printSection) {
			...
			case '1T':
				...
				break;
			case '2N':
				console.log('skrrt skrrt' + (skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2);
				console.log('broom: ' + (mold_length - (skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)));
				// make arg for boards that go into 3n & 3t
				skateboard = skateboard.intersect(color([0.99, 0, 0], cube({ size: [printLength, sectionWidth, sectionHeight], radius: [0.2, 0.2, 0.2], center: true, fn: 32 })).translate([printLength*2 + (nose_length - tail_length) / 2 , sectionWidthMoveX, 0]));
				break;
			case '2T':
				skateboard = skateboard.intersect(color([0.99, 0, 0], cube({ size: [printLength, sectionWidth, sectionHeight], radius: [0.2, 0.2, 0.2], center: true, fn: 32 })).translate([(-printLength*2 + (nose_length - tail_length) / 2), sectionWidthMoveX, 0]));
				break;
		...
		switch (printSection) {
			...
			case '2N':
				// make arg for boards that go into 3n & 3t
				skateboard = skateboard.intersect(color([0.99, 0, 0], intersection(cube({ size: [printLength, sectionWidth, mold_height], radius: [0.2, 0.2, 0], center: true, fn: 32 }).translate([(printLength*2 + (nose_length - tail_length) / 2), sectionWidthMoveX, -mold_height / 2]), cube({ size: [printLength, sectionWidth, mold_height], center: true, fn: 32 }).translate([(printLength*2 + (nose_length - tail_length) / 2), sectionWidthMoveX, -mold_height / 2 + 0.3 / 25.4]))));
				break;
			case '2T':
				skateboard = skateboard.intersect(color([0.99, 0, 0], intersection(cube({ size: [printLength, sectionWidth, mold_height], radius: [0.2, 0.2, 0], center: true, fn: 32 }).translate([(-printLength*2 + (nose_length - tail_length) / 2), sectionWidthMoveX, -mold_height / 2]), cube({ size: [printLength, sectionWidth, mold_height], center: true, fn: 32 }).translate([(-printLength*2 + (nose_length - tail_length) / 2), sectionWidthMoveX, -mold_height / 2 + 0.3 / 25.4]))));
				break;
...
function doZipties (skateboard, display, router_template, nose_length, tail_length, mold_length, mold_width, mold_height, mold_offset, printLength, printSection, nose_nose_male_ziptie, anose_nose_male_ziptie, center_male_ziptie, atail_tail_male_ziptie, tail_tail_male_ziptie, nose_nose_female_ziptie, anose_nose_female_ziptie, center_female_ziptie, atail_tail_female_ziptie, tail_tail_female_ziptie, ziptie_recess_depth, ziptie_recess_width) {
	...
	var alingmentHoleOffset = 7.72441/25.4;
	var alignmentNutWidth = 9.85 / 25.4;
	var alignmentNutHeight = 11.2 / 25.4;
	var alignmentBoltLength = 10 / 25.4;

	if (display == '1' || display == '2') {
		var verticalPlane = CSG.Plane.fromPoints([0, 0, 0], [5, 0, 1], [-5, 0, 1]);
		var ziptie_array = [];

		if (display == '1' && !router_template) {
			// nose to nose
			...
			ziptie_array.push(cylinder({ r: alignmentHoleDiameter/2, h: wall_width, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 + (mold_length / 2) - wall_width/2, alignmentHoleDistance/2, -mold_height + alingmentHoleOffset]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [alignmentBoltLength, alignmentNutWidth, (alingmentHoleOffset + alignmentNutHeight/2)], center: true }).translate([(nose_length - tail_length) / 2 + (mold_length / 2) - alignmentBoltLength/2 - wall_width, alignmentHoleDistance/2, -mold_height + (alingmentHoleOffset + alignmentNutHeight/2)/2]));
			...
			// nose to nose
			ziptie_array.push(cylinder({r: (outerDiameter/2+ziptie_clearance), h: nose_nose_female_ziptie, center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + router_template, 0, nose_nose_female_ziptie/2+skateboard.getBounds()[1].z-nose_nose_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({r: innerDiameter/2, h: nose_nose_female_ziptie, center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + router_template, 0, nose_nose_female_ziptie/2+skateboard.getBounds()[1].z-nose_nose_female_ziptie]));
			
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [wall_width, outerDiameter+ziptie_clearance*2, (nose_nose_female_ziptie-ziptie_height*2)/5], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-wall_width/2, 0, skateboard.getBounds()[1].z-((nose_nose_female_ziptie-ziptie_height*2)/5)/2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [wall_width, outerDiameter+ziptie_clearance*2, (nose_nose_female_ziptie-ziptie_height*2)/5], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-wall_width/2, 0, skateboard.getBounds()[1].z-nose_nose_female_ziptie+((nose_nose_female_ziptie-ziptie_height*2)/5)/2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [wall_width, outerDiameter+ziptie_clearance*2, ((nose_nose_female_ziptie-ziptie_height*2)/5)*3], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-wall_width/2, 0, skateboard.getBounds()[1].z-((nose_nose_female_ziptie-ziptie_height*2)/5+ziptie_height)-(((nose_nose_female_ziptie-ziptie_height*2)/5)*3)/2]));
			
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [outerDiameter+ziptie_clearance*2, wall_width, (nose_nose_female_ziptie-ziptie_height*2)/5], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width/2, skateboard.getBounds()[1].z-((nose_nose_female_ziptie-ziptie_height*2)/5)/2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [outerDiameter+ziptie_clearance*2, wall_width, (nose_nose_female_ziptie-ziptie_height*2)/5], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width/2, skateboard.getBounds()[1].z-nose_nose_female_ziptie+((nose_nose_female_ziptie-ziptie_height*2)/5)/2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [outerDiameter+ziptie_clearance*2, wall_width, ((nose_nose_female_ziptie-ziptie_height*2)/5)*3], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width/2, skateboard.getBounds()[1].z-((nose_nose_female_ziptie-ziptie_height*2)/5+ziptie_height)-(((nose_nose_female_ziptie-ziptie_height*2)/5)*3)/2]));	
			
			ziptie_array.push(cylinder({r: (outerDiameter/2+ziptie_clearance), h: ziptie_height, center: true}).rotateY(90).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-ziptie_height/2-outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({r: innerDiameter/2, h: ziptie_height-slice, center: true}).rotateY(90).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-ziptie_height/2-outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-ziptie_height/2-outerDiameter, 0, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({size: [ziptie_height, wall_width, innerDiameter/2], center: true}).translate([((nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2-ziptie_height/2-outerDiameter, wall_width/2, skateboard.getBounds()[1].z - innerDiameter/4 - ziptie_recess_depth]));

			ziptie_array.push(cylinder({ r: alignmentHoleDiameter/2, h: wall_width, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 + (mold_length / 2) - wall_width/2, alignmentHoleDistance/2, skateboard.getBounds()[1].z - alingmentHoleOffset]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [alignmentBoltLength, alignmentNutWidth, (alingmentHoleOffset + alignmentNutHeight/2)], center: true }).translate([(nose_length - tail_length) / 2 + (mold_length / 2) - alignmentBoltLength/2 - wall_width, alignmentHoleDistance/2, skateboard.getBounds()[1].z - (alingmentHoleOffset + alignmentNutHeight/2)/2]));
			
			// anose to nose
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: anose_nose_female_ziptie, center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y + router_template, anose_nose_female_ziptie / 2 + skateboard.getBounds()[1].z - anose_nose_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: anose_nose_female_ziptie, center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y + router_template, anose_nose_female_ziptie / 2 + skateboard.getBounds()[1].z - anose_nose_female_ziptie]));
			...
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (anose_nose_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y, skateboard.getBounds()[1].z - ((anose_nose_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (anose_nose_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y, skateboard.getBounds()[1].z - anose_nose_female_ziptie + ((anose_nose_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, ((anose_nose_female_ziptie - ziptie_height * 2) / 5) * 3], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y, skateboard.getBounds()[1].z - ((anose_nose_female_ziptie - ziptie_height * 2) / 5 + ziptie_height) - (((anose_nose_female_ziptie - ziptie_height * 2) / 5) * 3) / 2]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateY(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateY(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [ziptie_height, wall_width, innerDiameter / 2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, wall_width / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			
			if (router_template) {
				ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([2 * printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			}


			// center
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: center_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 + (printLength / 2), -skateboard.getBounds()[0].y + router_template, center_female_ziptie / 2 + skateboard.getBounds()[1].z - center_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: center_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 + (printLength / 2), -skateboard.getBounds()[0].y + router_template, center_female_ziptie / 2 + skateboard.getBounds()[1].z - center_female_ziptie]));
			...
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (center_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - ((center_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (center_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - center_female_ziptie + ((center_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, ((center_female_ziptie - ziptie_height * 2) / 5) * 3], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - ((center_female_ziptie - ziptie_height * 2) / 5 + ziptie_height) - (((center_female_ziptie - ziptie_height * 2) / 5) * 3) / 2]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: center_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 - (printLength - (printLength / 2)), -skateboard.getBounds()[0].y + router_template, center_female_ziptie / 2 + skateboard.getBounds()[1].z - center_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: center_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 - (printLength - (printLength / 2)), -skateboard.getBounds()[0].y + router_template, center_female_ziptie / 2 + skateboard.getBounds()[1].z - center_female_ziptie]));
			...
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (center_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2) + wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - ((center_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (center_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2) + wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - center_female_ziptie + ((center_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, ((center_female_ziptie - ziptie_height * 2) / 5) * 3], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2) + wall_width / 2, -skateboard.getBounds()[0].y + router_template, skateboard.getBounds()[1].z - ((center_female_ziptie - ziptie_height * 2) / 5 + ziptie_height) - (((center_female_ziptie - ziptie_height * 2) / 5) * 3) / 2]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateY(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateY(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, 0, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [ziptie_height, wall_width, innerDiameter / 2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2) - ziptie_height / 2 - outerDiameter, wall_width / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 - (printLength / 2) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 - (printLength / 2) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [ziptie_height, wall_width, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2) + ziptie_height / 2 + outerDiameter, wall_width / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			...
			// here
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

			if (router_template) {
				ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([printLength + (nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

				ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength / 2), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			}


			// atail to tail
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: atail_tail_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y + router_template, atail_tail_female_ziptie / 2 + skateboard.getBounds()[1].z - atail_tail_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: atail_tail_female_ziptie, center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y + router_template, atail_tail_female_ziptie / 2 + skateboard.getBounds()[1].z - atail_tail_female_ziptie]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [ziptie_height, wall_width, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)) + ziptie_height / 2 + outerDiameter, wall_width / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			...
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), ziptie_height / 2 + outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

			if (router_template) {
				ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateX(90).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - ziptie_recess_depth]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [outerDiameter + ziptie_clearance * 2, ziptie_recess_width, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
				ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width * 2, ziptie_height, innerDiameter / 2], center: true }).translate([(nose_length - tail_length) / 2 - (printLength + (printLength / 2)), -skateboard.getBounds()[0].y - ziptie_height / 2 - outerDiameter, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));
			}
			
			// tail to tail
			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: tail_tail_female_ziptie, center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 - router_template, 0, tail_tail_female_ziptie / 2 + skateboard.getBounds()[1].z - tail_tail_female_ziptie]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: tail_tail_female_ziptie, center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 - router_template, 0, tail_tail_female_ziptie / 2 + skateboard.getBounds()[1].z - tail_tail_female_ziptie]));

			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (tail_tail_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + wall_width / 2, 0, skateboard.getBounds()[1].z - ((tail_tail_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, (tail_tail_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + wall_width / 2, 0, skateboard.getBounds()[1].z - tail_tail_female_ziptie + ((tail_tail_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [wall_width, outerDiameter + ziptie_clearance * 2, ((tail_tail_female_ziptie - ziptie_height * 2) / 5) * 3], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + wall_width / 2, 0, skateboard.getBounds()[1].z - ((tail_tail_female_ziptie - ziptie_height * 2) / 5 + ziptie_height) - (((tail_tail_female_ziptie - ziptie_height * 2) / 5) * 3) / 2]));

			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [outerDiameter + ziptie_clearance * 2, wall_width, (tail_tail_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width / 2, skateboard.getBounds()[1].z - ((tail_tail_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [outerDiameter + ziptie_clearance * 2, wall_width, (tail_tail_female_ziptie - ziptie_height * 2) / 5], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width / 2, skateboard.getBounds()[1].z - tail_tail_female_ziptie + ((tail_tail_female_ziptie - ziptie_height * 2) / 5) / 2]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [outerDiameter + ziptie_clearance * 2, wall_width, ((tail_tail_female_ziptie - ziptie_height * 2) / 5) * 3], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2, wall_width / 2, skateboard.getBounds()[1].z - ((tail_tail_female_ziptie - ziptie_height * 2) / 5 + ziptie_height) - (((tail_tail_female_ziptie - ziptie_height * 2) / 5) * 3) / 2]));

			ziptie_array.push(cylinder({ r: (outerDiameter / 2 + ziptie_clearance), h: ziptie_height, center: true }).rotateY(90).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + (outerDiameter / 2 + ziptie_clearance) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cylinder({ r: innerDiameter / 2, h: ziptie_height - slice, center: true }).rotateY(90).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + (outerDiameter / 2 + ziptie_clearance) + ziptie_height / 2 + outerDiameter, 0, skateboard.getBounds()[1].z - ziptie_recess_depth]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [ziptie_recess_width, outerDiameter + ziptie_clearance * 2, innerDiameter / 2 + ziptie_recess_depth/2], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + (outerDiameter / 2 + ziptie_clearance) + ziptie_height / 2 + outerDiameter, 0 / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth/4]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].subtract(cube({ size: [ziptie_height, wall_width, innerDiameter / 2], center: true }).translate([-(-(nose_length-tail_length) + skateboard.getBounds()[1].x - skateboard.getBounds()[0].x)/2 + (outerDiameter / 2 + ziptie_clearance) + ziptie_height / 2 + outerDiameter, wall_width / 2, skateboard.getBounds()[1].z - innerDiameter / 4 - ziptie_recess_depth]));

			ziptie_array.push(cylinder({ r: alignmentHoleDiameter/2, h: wall_width, center: true }).rotateY(90).translate([(nose_length - tail_length) / 2 - (mold_length / 2) + wall_width/2, alignmentHoleDistance/2, skateboard.getBounds()[1].z - alingmentHoleOffset]));
			ziptie_array[ziptie_array.length - 1] = ziptie_array[ziptie_array.length - 1].union(cube({ size: [alignmentBoltLength, alignmentNutWidth, (alingmentHoleOffset + alignmentNutHeight/2)], center: true }).translate([(nose_length - tail_length) / 2 - (mold_length / 2) + alignmentBoltLength/2 + wall_width, alignmentHoleDistance/2, skateboard.getBounds()[1].z - (alingmentHoleOffset + alignmentNutHeight/2)/2]));
		}
		
		//var skateboardPerimeter = skateboard.subtract((skateboard.scale([1 - (Math.abs(skateboard.getBounds()[0].x)-(Math.abs(skateboard.getBounds()[0].x)-1/25.4))/((Math.abs(skateboard.getBounds()[0].x)+(Math.abs(skateboard.getBounds()[0].x)-1/25.4))/2), 1 - (Math.abs(skateboard.getBounds()[0].x)-(Math.abs(skateboard.getBounds()[0].x)-1/25.4))/((Math.abs(skateboard.getBounds()[0].y)+(Math.abs(skateboard.getBounds()[0].y)-1/25.4))/2),1])));
